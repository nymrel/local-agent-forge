...
class TaskClassifier {
    ...
    classify(prompt, context) {
        ...
        let complexity;
        if (score <= 0.20) {
            complexity = types_js_1.TaskComplexity.TRIVIAL;
        }
        else if (score <= 0.50) {
            complexity = types_js_1.TaskComplexity.ROUTINE;
        }
        else if (score <= 0.70) {
            complexity = types_js_1.TaskComplexity.MODERATE;
        }
        else if (score <= types_js_1.REASONING_ESCALATION_THRESHOLD) {
            complexity = types_js_1.TaskComplexity.COMPLEX;
        }
        else {
            complexity = types_js_1.TaskComplexity.FRONTIER_REASONING;
        }
        // Determine category
        ...
        const recommendedCloudModel = score > types_js_1.REASONING_ESCALATION_THRESHOLD ? 'claude-3-5-sonnet' : 'gpt-4o-mini';
        const rationale = score > types_js_1.REASONING_ESCALATION_THRESHOLD
            ? `Task complexity score (${score}) exceeds ${types_js_1.REASONING_ESCALATION_THRESHOLD * 100}% threshold due to ${triggers.filter(t => t.startsWith('high:')).join(', ') || 'deep multi-layered reasoning requirements'}. Escalating to cloud frontier.`
            : `Task complexity score (${score}) is within local GPU capability envelope (<= ${types_js_1.REASONING_ESCALATION_THRESHOLD * 100}%). Routing to local GPU at $0 token cost.`;
        return {
            score,
            complexity,
            triggers,
    ...
    determineCategory(text, triggers) {
        if (triggers.some(t => t.startsWith('high:')))
            return 'deep_reasoning';
        if (/\b(test|jest|pytest|assert|spec)\b/i.test(text))
            return 'unit_test';
        if (/\b(doc|comment|readme|jsdoc)\b/i.test(text))
            return 'documentation';
        if (/\b(bug|fix|error|exception|debug|traceback)\b/i.test(text))
            return 'debugging';
        if (/\b(refactor|cleanup|simplify|restructure)\b/i.test(text))
            return 'refactor';
        if (/\b(function|class|code|implement|create|build|api|endpoint)\b/i.test(text))
            return 'code_gen';
        return 'general';
    }
    recommendLocalModel(category, complexity) {
        switch (category) {
            case 'code_gen':
            case 'refactor':
            case 'debugging':
            case 'unit_test':
                return complexity === types_js_1.TaskComplexity.COMPLEX ? 'qwen2.5-coder:14b' : 'qwen2.5-coder:7b';
            case 'deep_reasoning':
