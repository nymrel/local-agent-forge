...
import { CLOUD_BASELINES, DEFAULT_BASELINE_MODEL, ModelPricing, calculateCost } from './baselines.js';
...
export class TokenLedger {
  ...
  private baselineModelKey: string;

  constructor(defaultBaseline: string = DEFAULT_BASELINE_MODEL) {
    this.baselineModelKey = defaultBaseline;
  }

  setBaselineModel(modelKey: string): void {
    if (CLOUD_BASELINES[modelKey]) {
      this.baselineModelKey = modelKey;
    }
  }

  getBaselineModel(): ModelPricing {
    return CLOUD_BASELINES[this.baselineModelKey] || CLOUD_BASELINES[DEFAULT_BASELINE_MODEL];
  }
  ...
  record(entry: {
    ...
    const baselinePricing = CLOUD_BASELINES[baselineKey] || this.getBaselineModel();
    const localPricing = CLOUD_BASELINES['local-gpu'];
    ...
    const actualCost = entry.isLocal
      ? calculateCost(promptTokens, completionTokens, localPricing)
      : calculateCost(promptTokens, completionTokens, baselinePricing);

    const baselineCost = calculateCost(promptTokens, completionTokens, baselinePricing);
...
`.trim();
  ...
  importJson(jsonStr: string): void {
    try {
      ...
      if (data.baseline && CLOUD_BASELINES[data.baseline]) {
